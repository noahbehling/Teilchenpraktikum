...
The data samples used for the analysis have been preselected with either an electron or muon trigger. In the preselection a cut on the lepton and jet transverse momentum
$\texttt{lep\_pt}, \, \texttt{jet\_pt} > 25 \, \si{\giga\eV}$ and on the pseudorapidity for jets and lepton of $|\eta| <2.5$ has been applied.
The samples are saved in \texttt{ntuples}. Sometimes an entry for a variable contains more entries than the \texttt{ntuple} itself since for some variables a new entry is created for every lepton or jet if multiple electrons or jets are measured. An example for such a variable is the lepton transverse momentum
shown in Figure \ref{fig:unselected_pt} with 467238 entries while the data sample has 450199 entries.
The lepton transverse momentum rises sharply at low momenta and has a peak arround $\SI{50}{\giga\eV}$. After that it falls exponentially.
...
decay of a W boson. The selection consists of 7 cuts which are applied to tha data samples subsequentially.
The efficiency of the selection $(A \cdot \epsilon) = \# \text{events(after cut)} / \# \text{events(before cut)}$ of every cut together with all
cuts that have been applied beforehand is listed in Tables \ref{tab:eff_a} and \ref{tab:eff_b} for all Monte Carlo samples.\\
...
The main background after the selection is $t \overline{t}$-production due to its similar final state which leads to a large selection efficiency
...
expected. The same is observed for the kinematic quantities of the jet with the largest transverse momentum of each event. All considered variables follow
...
for the $t \overline{t}$-sample no function describing the decrease of the $Z^\prime$-sample is obvious. In distribution 4 both samples show a peak
...
hypothetic particle. Since discriminant 4 should roughly correspond to the invariant mass of the final state of the decay considered we would expect
a peak in the distribution at the mass of a hypothetic $Z^\prime$. Due to this clear signature of a possible observation discriminant 4 is chosen as
...
%backgrounds from the standard model would be seen as statistically significant.
